...
// sources at run time, and the simtgo vet analyzer, which is handed the real
...
type Unit struct {
	...
	SourceHash string
	// FastMath says the kernel carried //simtgo:fastmath, and is what tells a
...
type transpiler struct {
	...
	sizes types.Sizes
	// allowFloat64 is set from the kernel's //simtgo:float64 directive, and
	// kernelName is whose directive it would have been, for the message.
	allowFloat64 bool
	kernelName   string
	// fastMath is set from the kernel's //simtgo:fastmath directive. Unlike
...
func (t *transpiler) kernel(fd *ast.FuncDecl) {
	...
	t.line("// generated by github.com/CWBudde/simtgo/simt from %s", t.fset.Position(fd.Pos()).Filename)
	if t.fastMath {
...
//
// Two names come from here rather than from the Go source: simtgo_padN, the
// explicit padding a struct's holes are written as, and simtgo_bounds, the
...
// author's are the same C symbol. A local named simtgo_bounds shadows the
...
// on. A build option must not move the subset: a kernel that `simtgo vet`
...
const boundsHelper = "simtgo_bounds"

const boundsHelperDef = `__device__ __forceinline__ long long simtgo_bounds(long long i, long long n)
{
	if (i < 0 || i >= n)
	{
		__trap();
