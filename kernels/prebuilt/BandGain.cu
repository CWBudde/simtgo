// generated by github.com/CWBudde/simtgo/simt from bandgain.go
struct Band
{
	float Upper;
	float Gain;
};
static_assert(sizeof(Band) == 8, "simtgo: Band is a different size in CUDA than in Go");
static_assert(alignof(Band) == 4, "simtgo: Band is differently aligned in CUDA than in Go");
