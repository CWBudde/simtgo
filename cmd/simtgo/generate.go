...
func generate(args []string) int {
	...
	fs := flag.NewFlagSet("simtgo generate", flag.ExitOnError)
	...
	fs.Usage = func() {
		fmt.Fprint(fs.Output(), `usage: simtgo generate [flags]
...
`)
	...
	if err := run(o); err != nil {
		fmt.Fprintf(os.Stderr, "simtgo generate: %v\n", err)
		return 1
	}
	return 0
}
...
func renderGen(o generateOptions, units []*lower.Unit, compiled map[string]compiledPTX) []byte {
	...
	fmt.Fprintf(&b, "// Code generated by \"simtgo generate\"; DO NOT EDIT.\n\n")
...
// import nothing but simtgo/gpu, and everything here imports simt.
package %s

// Lowered names a kernel that "simtgo generate" was able to lower to CUDA C.
type Lowered string

// Gate lists every kernel that must lower for this package to compile.
//
// Each name is a constant in %s, which "simtgo generate"
