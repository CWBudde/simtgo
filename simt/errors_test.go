...
// a distinctive phrase from the diagnostic it must draw.
//
// It is a named type at package scope rather than an anonymous struct inside
// the test because SPEC.md is checked against it: spec_test.go ranges these
// cases and fails if the document does not carry every want below. A refusal
// the code enforces and the contract does not mention is the drift that made
// writing the contract worth doing.
...
// The transpiler covers a subset of Go, and this runs every case in refusals
// against it.
func TestUnsupported(t *testing.T) {
	for _, tc := range refusals {
		t.Run(tc.name, func(t *testing.T) {
			fsys := fstest.MapFS{"k.go": &fstest.MapFile{Data: []byte(refusalPrelude + tc.body + "\n")}}
